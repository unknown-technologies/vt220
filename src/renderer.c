...
#define	BLUR_WIDTH_80		SCREEN_WIDTH_80
#define	BLUR_WIDTH_132		SCREEN_WIDTH_132
...
static const GLfloat vt220_colors[4][3] = {
	/* monochrome */
#ifndef VT220_ALTERNATE_INTENSITY
...
void VTInitRenderer(VTRenderer* self, VT220* vt)
{
	...
	self->post_shader_fbtex = glGetUniformLocation(self->post_shader, "vt220_screen");
...
void VTRender(VTRenderer* self, unsigned int width, unsigned int height)
{
	...
	self->was_132col = self->vt->mode & DECCOLM;
}

void VTRenderTerminal(VTRenderer* self)
{
	unsigned int width = self->vt->mode & DECCOLM ? SCREEN_WIDTH_132 : SCREEN_WIDTH_80;
	bool resize = !!(self->vt->mode & DECCOLM) != self->was_132col;
	...
	glUniform3fv(self->vt_shader_colorscheme, 8, (GLfloat*) vt220_colors);
...
void VTRenderBlur(VTRenderer* self)
{
	...
	if(resize) {
		for(unsigned int i = 0; i < 2; i++) {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, BLUR_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		}
	}
	...
	glClear(GL_COLOR_BUFFER_BIT);
	glViewport(0, 0, width, BLUR_HEIGHT);
...
void VTCreateBlurFrameBuffer(VTRenderer* self)
{
	...
	for(unsigned int i = 0; i < 2; i++) {
		glBindTexture(GL_TEXTURE_2D, self->blur_tex[i]);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, BLUR_WIDTH_80, BLUR_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
